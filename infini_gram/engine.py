...
class InfiniGramEngine:
    ...
    def __init__(self, index_dir, eos_token_id, **kwargs):
        ...
        consts = cpp_engine.Consts(
            MAX_CNT_FOR_NTD, MAX_OUTPUT_DOC_TOKENS, MAX_CLAUSE_FREQ_PER_SHARD, MAX_DIFF_TOKENS, MAX_DIFF_BYTES,
        )
        ...
        if type(index_dir) == str:
            self.lm = cpp_engine.NGramLanguageModeling(
                index_dir, eos_token_id, ds_prefetch_depth, sa_prefetch_depth, od_prefetch_depth, consts,
            )
        elif type(index_dir) == list and all(type(d) == str for d in index_dir):
            self.lm = cpp_engine.NGramLanguageModelingUnion(
                index_dir, eos_token_id, ds_prefetch_depth, sa_prefetch_depth, od_prefetch_depth, consts,
            )
        else:
    ...
    def count(self, query_ids):
        if self.check_query_ids(query_ids, allow_empty=True):
            result = self.lm.find(input_ids=query_ids)
        else: # cnf
            ...
            result = self.lm.find_cnf(cnf=query_ids)
    ...
    def prob(self, query_ids):
        ...
        return {'prob': result.prob, 'prompt_cnt': result.prompt_cnt, 'cont_cnt': result.cont_cnt}
    ...
    def ntd(self, query_ids):
        ...
        for token_id in result.prob_by_token_id:
            ...
            result_by_token_id[token_id] = {'prob': prob, 'cont_cnt': freq}
        return {'prompt_cnt': result.prompt_cnt, 'result_by_token_id': result_by_token_id}

    def infgram_prob(self, query_ids):
        if not self.check_query_ids(query_ids, allow_empty=False):
            return {'error': 'query_ids must be a non-empty list of integers in range [0, 65536)'}
        result = self.lm.infgram_prob(prompt_ids=query_ids[:-1], cont_id=query_ids[-1])
        return {'prob': result.prob, 'prompt_cnt': result.prompt_cnt, 'cont_cnt': result.cont_cnt, 'suffix_len': result.lfn - 1}

    def infgram_ntd(self, query_ids):
        if not self.check_query_ids(query_ids, allow_empty=True):
            return {'error': 'query_ids must be a list of integers in range [0, 65536)'}
        result = self.lm.infgram_ntd(prompt_ids=query_ids[:-1])
        result_by_token_id = {}
        for token_id in result.prob_by_token_id:
            prob = result.prob_by_token_id[token_id]
            freq = result.freq_by_token_id[token_id]
            result_by_token_id[token_id] = {'prob': prob, 'cont_cnt': freq}
        return {'prompt_cnt': result.prompt_cnt, 'result_by_token_id': result_by_token_id, 'suffix_len': result.lfn - 1}
