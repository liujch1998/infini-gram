...
// table: 4t bytes; text: 2t bytes; size_width = 4
...
fn cmd_merge(data_files: &Vec<String>, merged_dir: &String, num_threads: i64, hacksize: usize)  -> std::io::Result<()> {
    ...
    let metadatas:Vec<u64> = (0..nn).map(|x| {
        ...
        assert!(meta.len()%(texts[x].len() as u64 / 2) == 0);
        return meta.len();
    }).collect();
    ...
    let ratio = metadatas[0] / (texts[0].len() as u64 / 2); // size of part pointer in bytes
